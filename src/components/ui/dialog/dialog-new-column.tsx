...
interface DialogNewColumnProps {
  open: boolean;
  setOpen: (value: boolean) => void;
}

export default function DialogNewColumn({ open, setOpen }: DialogNewColumnProps) {
  ...
  const handleCreateColumn = () => {
    ...
    const newColumn = { id: columns.length + 1, nome: name, cards: [], conclusionPercentage: 0 };
    const updatedColumns = [...columns, newColumn];
    recalculateColumnPercentages(updatedColumns);
  ...
  const recalculateColumnPercentages = (columns: IColunas[]) => {
    const numberOfColumns = columns.length;
    columns.forEach((column, index) => {
      column.conclusionPercentage = Math.ceil(((index / (numberOfColumns - 1)) * 100));
                ...
                <div className="bg-bgcool px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                  <div className="mb-4">
                    ...
                    <input
                      ...
                      id="columnName"
                      name="column" placeholder="Insira o nome da coluna..." type="text" className="w-full bg-bgcool border-2 mb-2 border-mem rounded-md p-1" />
                  </div>
                </div>
